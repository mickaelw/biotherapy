...
namespace Back\CardBundle\Controller;

use Symfony\Bundle\FrameworkBundle\Controller\Controller;
use Front\WebsiteBundle\Entity\Card;
use Front\WebsiteBundle\Form\CardType;
...
class DefaultController extends Controller
{
    public function indexAction()
    {
        ...
        return $this->render('BackCardBundle::index.html.twig', array(
        	'list'	=> $listCard,
        ));
    }
    ...
    public function addAction(){
    	$objCard 	= new Card();
    	$form 		= $this->createForm(new CardType, $objCard);
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($form->isValid()) {
				$em = $this->getDoctrine()->getManager();
				$objCard->setCreatedTime(new \Datetime());
				$objCard->setModifiedTime(new \Datetime());
				$em->persist($objCard);
				$em->flush();
				
				$this->get('session')->getFlashBag()->add('done', 'La fiche a bien été ajoutée');
				
				return $this->redirect($this->generateUrl('back_card_homepage'));
			}
		}
    	
        return $this->render('BackCardBundle:Form:add.html.twig', array(
    ...
    {
	    $em 		= $this->getDoctrine()->getManager();
    	$objCard 	= $em->getRepository('FrontWebsiteBundle:Card')->find($id);
    	$form 		= $this->createForm(new CardType, $objCard);
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($form->isValid()) {
				...
				$this->get('session')->getFlashBag()->add('done', 'La fiche a bien été modifiée');
				
				return $this->redirect($this->generateUrl('back_card_homepage'));
			}
		}
    	
        return $this->render('BackCardBundle:Form:update.html.twig', array(
        	'obj'	=> $objCard,
        	'form'	=> $form->createView()
        ));
    }
    
    public function miniAction($id,$slug,$format)
    {
	    $em 		= $this->getDoctrine()->getManager();
    	$objCard 	= $em->getRepository('FrontWebsiteBundle:Card')->find($id);
    	$listCard 	= $em->getRepository('FrontWebsiteBundle:Card')->findAll();
    	$listBio 	= $em->getRepository('FrontWebsiteBundle:Biotherapy')->findAll();
    	$listCS 	= $em->getRepository('FrontWebsiteBundle:ClinicalSituation')->findBy(array('parent' => null));
        ...
        return $this->render('BackCardBundle:Form:update.mini.html.twig', array(
        	'card'		=> $objCard,
        	'listBio'	=> $listBio,
        	'listCS'	=> $listCS,
        	'listKeyWords'	=> $names,
    ...
    public function miniUpdateAction($id)
    {
	    $em 		= $this->getDoctrine()->getManager();
    	$objCard 	= $em->getRepository('FrontWebsiteBundle:Card')->find($id);
    	$form 		= $this->createForm(new CardType, $objCard);
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($form->isValid()) {
				...
				return $this->redirect($this->generateUrl('front_website_card_admin', array(
					'id'		=> $id,
					'slug'		=> 'test',
					'format'	=> 'html'
				)));
    ...
    public function deleteAction($id)
    {
		...
		$this->get('session')->getFlashBag()->add('error', 'La fiche a bien été supprimée');
		return $this->redirect($this->generateUrl('back_card_homepage'));
    }
